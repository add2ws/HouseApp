...
public class PaiHangmjActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_paihang_xsmj);
		...
		textHouse.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				 new AlertDialog.Builder(PaiHangmjActivity.this)
				 .setTitle("请选择房屋类型")
	...
	//客服端获取值传给服务端后台
		class readData extends AsyncTask<Parameter, Void, String>{
			...
			protected String doInBackground(Parameter... param) {
				String result = null;
				try {
					result = HttpHelper.requestData(HouseConfig.METHOD_PAIHANGMJ, C.Url, param[0]);
				} catch (IOException e) {
					e.printStackTrace();
				}
				return result;
			...
			protected void onPostExecute(String result) {
				...
				if (TextUtils.isEmpty(result)) {
					Toast.makeText(PaiHangmjActivity.this, "网络异常,请稍后再试", Toast.LENGTH_SHORT).show();
				} else {
					try {
						...
						MyAdapter adp= new MyAdapter(PaiHangmjActivity.this, list,
								R.layout.paihang_item_xsmj, new String[]
										{"titleName","rowMonth","mianji","name","place","jj","rowYear"}, new int[]
										{R.id.title_name,R.id.rowMonth,R.id.mianji,R.id.name,R.id.place,R.id.jj,R.id.rowYear});
