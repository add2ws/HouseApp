...
public class PaiHangjjActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_paihang_xsjj);
		...
		textHouse.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				 new AlertDialog.Builder(PaiHangjjActivity.this)
				 .setTitle("请选择房屋类型")
	...
	//客服端获取值传给服务端后台
		class readData extends AsyncTask<Parameter, Void, String>{
			...
			protected String doInBackground(Parameter... param) {
				String result = null;
				try {
					result = HttpHelper.requestData(HouseConfig.METHOD_PAIHANGJJ, C.Url, param[0]);
				} catch (IOException e) {
					e.printStackTrace();
				}
				return result;
			...
			protected void onPostExecute(String result) {
				...
				if (TextUtils.isEmpty(result)) {
					Toast.makeText(PaiHangjjActivity.this, "网络异常,请稍后再试", Toast.LENGTH_SHORT).show();
				} else {
					try {
						...
						for(int i=0;i<jsonArray.length();i++){
							...
							String cjj = jsonObject.get("cjj").toString();
						...
						MyAdapter adp= new MyAdapter(PaiHangjjActivity.this, list,
								R.layout.paihang_item_xsjj, new String[]
										{"titleName","rowMonth","cjj","name","place","jj","rowYear"}, new int[]
										{R.id.title_name,R.id.rowMonth,R.id.cjj,R.id.name,R.id.place,R.id.jj,R.id.rowYear});
