...
public class ZhuzhaiMianjiActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_zhuzhai_mianji);
		...
		backBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				ZhuzhaiMianjiActivity.this.finish();
	...
	//客服端获取值传给服务端后台
		class readData extends AsyncTask<Parameter, Void, String>{
			...
			protected String doInBackground(Parameter... param) {
				String result = null;
				try {
					result = HttpHelper.requestData(HouseConfig.METHOD_GETMJFB, C.Url, param[0]);
				} catch (IOException e) {
					e.printStackTrace();
				}
				return result;
			...
			protected void onPostExecute(String result) {
				...
				if (TextUtils.isEmpty(result)) {
					Toast.makeText(ZhuzhaiMianjiActivity.this, "网络异常,请稍后再试", Toast.LENGTH_SHORT).show();
				} else {
					try {
						...
						for(int i=0;i<jsonArray.length();i++){
							...
							String buildingArea = jsonObject.get("buildingArea").toString();
						...
						MyAdapter adp= new MyAdapter(ZhuzhaiMianjiActivity.this, list,
								R.layout.zhuzhai_item_mianji, new String[]
										{"buildingArea","ssts","ssljts","ssbl","xsts","xsljts","xsbl","ksts","ksbl"}, new int[]
